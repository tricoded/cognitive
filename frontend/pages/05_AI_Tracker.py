...
# ─────────────────────────────────────────────────────────────────────────────
try:
    today_data = requests.get(f"{API}/ai-usage/today", timeout=4).json()
    ...
    total_today, ai_budget, pct_used, over_budget, today_logs = 0, 120, 0, False, []
...
def render_stats(days: int):
    try:
        stats = requests.get(f"{API}/ai-usage/stats?days={days}", timeout=4).json()
    except Exception:
        st.error("⚠️ Can't reach the backend.")
        return

    if stats.get("message") == "No data yet":
        st.info("No AI usage logged yet. Use the form above to track your first session!", icon="🤖")
        return
    ...
    st.markdown(
        f"""<div style="background:#1e1e2e; border-radius:10px; padding:16px; margin-top:8px;">
            ...
            <div style="margin-top:8px; font-size:0.82rem; color:{'#10b981' if act_pct >= 60 else '#f59e0b'};">
                {'✅ Great! Most of your AI use enhances your thinking.' if act_pct >= 60
                 else '⚠️ Most AI use is passive. Try to engage more critically with AI outputs.'}
            </div>
        </div>""",
        unsafe_allow_html=True,
    )
...
try:
    all_logs = requests.get(f"{API}/ai-usage/?days=30", timeout=4).json()
except Exception:
    all_logs = []

if not all_logs:
    st.info("No sessions logged yet.", icon="📭")
else:
...
# ─────────────────────────────────────────────────────────────────────────────
st.divider()
with st.expander("💡 Healthy AI Use Tips"):
    st.markdown("""
    **🧠 Active vs Passive**
    Active use = you think first, then use AI to enhance/verify/speed up.
    Passive use = you copy-paste with no critical engagement.
    Aim for **60%+ active use.**

    **⏱️ Time Budget**
    Research suggests **1-2 hours of focused AI use** per day is healthy for knowledge workers.
    Over-reliance reduces your own problem-solving ability.

    **📁 Category balance**
    If 90% of your AI use is one category, you might be outsourcing thinking you could build yourself.

    **🔄 Weekly review**
    Check your weekly AI use every Sunday. Ask: *"Did AI enhance my work or replace my thinking?"*
    """)
